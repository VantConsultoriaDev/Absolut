...
const Contratos: React.FC = () => {
  ...
  const handleGeneratePendingContracts = async () => {
    ...
    try {
      ...
      await getContracts();
      alert(`Contratos gerados: ${pendentes.length}`);
    } catch (err) {
      console.error('Erro ao gerar contratos pendentes:', err);
      alert('Falha ao gerar contratos pendentes. Verifique os dados e tente novamente.');
    } finally {
  ...
  const handleRegenerate = async (cargaId: string) => {
    try {
      ...
      alert('Não foi possível regerar o contrato.');
    }
  };
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">CRT</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Motorista</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Data</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Atualização</th>
              ...
              {filteredContratos.length === 0 ? (
                ...
                filteredContratos.map((contrato) => (
                  <tr key={contrato.id} className="table-body-row">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{contrato.crt || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{contrato.parceiroNome || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{contrato.motoristaNome || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{format(new Date(contrato.createdAt), 'dd/MM/yyyy HH:mm', { locale: ptBR })}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{format(new Date(contrato.updatedAt), 'dd/MM/yyyy HH:mm', { locale: ptBR })}</td>
